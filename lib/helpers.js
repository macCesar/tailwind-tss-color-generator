...
function resetStyles() {
	...
	convertedStyles += singleValue('Window', '', 'backgroundColor', `'#fff'`);

	convertedStyles += singleValue('ImageView[platform=ios]', '', 'hires', true);

	convertedStyles += `'View': { width: Ti.UI.SIZE, height: Ti.UI.SIZE }\n`;

	convertedStyles += singleValue('.vertical', '', 'layout', `'vertical'`);
	convertedStyles += singleValue('.horizontal', '', 'layout', `'horizontal'`);

	convertedStyles += singleValue('.vertical[platform=ios]', '', 'clipMode', 'Ti.UI.iOS.CLIP_MODE_DISABLED');
	convertedStyles += singleValue('.horizontal[platform=ios]', '', 'clipMode', 'Ti.UI.iOS.CLIP_MODE_DISABLED');
...
function fontWeight(fontWeights) {
	...
	_.each(fontWeights, (value, modifier) => {
		convertedStyles += singleValue('.font-', `${modifier}[platform=ios]`, 'font: { fontWeight', `'${fixInvalidValues(invalidValues, modifier)}' }`);
	});
...
function shadow(boxShadow) {
	...
	convertedStyles += `'.shadow-xs[platform=ios]': { viewShadowOffset: { x: 0, y: 0 }, viewShadowRadius: 1, viewShadowColor: '#77000000' }\n`;
	convertedStyles += `'.shadow-sm[platform=ios]': { viewShadowOffset: { x: 0, y: 1 }, viewShadowRadius: 1, viewShadowColor: '#77000000' }\n`;
	convertedStyles += `'.shadow[platform=ios]': { viewShadowOffset: { x: 0, y: 2 }, viewShadowRadius: 2, viewShadowColor: '#77000000' }\n`;
	convertedStyles += `'.shadow-md[platform=ios]': { viewShadowOffset: { x: 0, y: 4 }, viewShadowRadius: 4, viewShadowColor: '#77000000' }\n`;
	convertedStyles += `'.shadow-lg[platform=ios]': { viewShadowOffset: { x: 0, y: 8 }, viewShadowRadius: 8, viewShadowColor: '#77000000' }\n`;
	convertedStyles += `'.shadow-xl[platform=ios]': { viewShadowOffset: { x: 0, y: 12 }, viewShadowRadius: 12, viewShadowColor: '#77000000' }\n`;
	convertedStyles += `'.shadow-2xl[platform=ios]': { viewShadowOffset: { x: 0, y: 16 }, viewShadowRadius: 16, viewShadowColor: '#77000000' }\n`;
	convertedStyles += `'.shadow-inner[platform=ios]': { viewShadowOffset: { x: 0, y: 0 }, viewShadowRadius: null, viewShadowColor: null }\n`;
	convertedStyles += `'.shadow-outline[platform=ios]': { viewShadowOffset: { x: 0, y: 0 }, viewShadowRadius: 2, viewShadowColor: '#77000000' }\n`;
	...
	_.each(objectPosition, (properties, rule) => {
		...
		_.each(spacing, (value, modifier) => {
			...
			_.each(properties, property => {
				sides += (modifier === 'px')
					? ` ${property}: 1,`
					: (!modifier.includes('/')) ? ` ${property}: ${16 * parseFloat(value)},` : ` ${property}: '${value}',`;
			});
...
function processElements(header, rule, property, data) {
	...
	_.each(data.spacing, (value, modifier) => {
		// Ignore Tailwinds UI's fractional spacings.
		if (!modifier.includes("/")) {
			convertedStyles += (modifier === 'px')
				? singleValue(`.${rule}`, modifier, property, 1)
